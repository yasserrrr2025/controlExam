...
const AdminSupervisionMonitor: React.FC<Props> = ({ supervisions, users, students, absences, deliveryLogs }) => {
  ...
  const detailedStats = useMemo(() => {
    const committeeNums = Array.from(new Set(students.map(s => s.committee_number))).filter(Boolean).sort((a,b)=>Number(a)-Number(b)) as string[];
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                 ...
                 <div className="relative">
                    ...
                    <input type="date" className="w-full pr-14 p-5 bg-white/10 border border-white/10 rounded-2xl font-black outline-none focus:border-blue-500 transition-all text-white" value={reportInfo.date} onChange={e => setReportInfo({...reportInfo, date: e.target.value})} />
                 </div>
              </div>
              <div className="space-y-3">
                 ...
                 <div className="relative">
                    ...
                    <input type="text" placeholder="اكتب اسم المادة..." className="w-full pr-14 p-5 bg-white/10 border border-white/10 rounded-2xl font-black outline-none focus:border-blue-600 transition-all text-white" value={reportInfo.subject} onChange={e => setReportInfo({...reportInfo, subject: e.target.value})} />
                 </div>
              </div>
           </div>
           <button onClick={() => window.print()} className="w-full bg-blue-600 text-white py-6 rounded-[2rem] font-black text-2xl shadow-2xl hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center justify-center gap-5 active:scale-95">
                  ...
                  {detailedStats.map((stat, idx) => (
                    <tr key={idx} className={`hover:bg-blue-50/30 transition-colors ${stat.isDone ? 'bg-emerald-50/10' : ''}`}>
                       ...
                       <td className="p-6 text-center tabular-nums space-x-2 space-x-reverse text-xs">
                          ...
                          <span className="text-amber-600 bg-amber-50 px-2 py-1 rounded-md">ت:{stat.late}</span>
                       </td>
            ...
            <tr>
              <th colSpan={10} className="p-0 font-normal border-none text-right">
                ...
                <div className="text-center mt-2 mb-3">
                  <h2 className="text-[13pt] font-black border-b-[2pt] border-black pb-1 inline-block px-12 leading-none">مسير المراقبة ورصد حضور واستلام المظاريف</h2>
                  <div className="flex justify-center gap-10 text-[9.5pt] font-black mt-3 text-black">
                    ...
                    <span>المادة: <span className="border-b-2 border-black px-8">{reportInfo.subject || '................'}</span></span>
                  </div>
                </div>
                
                <table className="w-full border-collapse">
                  <tr className="bg-gray-100 font-black text-[8pt]">
                    <td className="cell-border" style={{ width: '8mm' }}>م</td>
                    <td className="cell-border" style={{ width: '13mm' }}>اللجنة</td>
                    <td className="cell-border text-right px-2" style={{ width: '45mm' }}>اسم المراقب</td>
                    <td className="cell-border" style={{ width: '28mm' }}>الصف</td>
                    <td className="cell-border" style={{ width: '10mm' }}>ح</td>
                    <td className="cell-border" style={{ width: '10mm' }}>غ</td>
                    <td className="cell-border" style={{ width: '10mm' }}>ت</td>
                    <td className="cell-border text-right px-2" style={{ width: '33mm' }}>المستلم</td>
                    <td className="cell-border" style={{ width: '20mm' }}>توقيع م</td>
                    <td className="cell-border" style={{ width: '20mm' }}>توقيع ك</td>
                  </tr>
                </table>
              </th>
            </tr>
          ...
          <tbody>
            {detailedStats.map((stat, i) => (
              <tr key={i}>
                ...
                <td className="cell-border font-black text-[9pt]" style={{ width: '13mm' }}>{stat.committee_number}</td>
                ...
                <td className="cell-border stat-cell" style={{ width: '10mm' }}>{stat.present}</td>
                <td className="cell-border stat-cell text-red-700" style={{ width: '10mm' }}>{stat.absent}</td>
                <td className="cell-border stat-cell" style={{ width: '10mm' }}>{stat.late}</td>
          ...
          <tfoot>
            <tr>
              <td colSpan={10} className="pt-8 border-none">
                <div className="grid grid-cols-2 text-[10pt] font-black text-center gap-24 px-16">
                ...
                <div className="text-left mt-6 text-[7pt] text-gray-400 italic px-4 border-t pt-1">
