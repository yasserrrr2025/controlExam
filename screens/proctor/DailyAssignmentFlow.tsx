    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto text-right pb-48 px-4 md:px-0">
       ...
       {isClosingWizardOpen && (
         <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 overflow-hidden animate-fade-in no-print bg-slate-900/40">
            <div className="absolute inset-0 backdrop-blur-md" onClick={() => !isVerifying && setIsClosingWizardOpen(false)}></div>
            
            <div className="bg-white w-full max-w-3xl max-h-[90vh] flex flex-col rounded-[3rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] relative z-10 border-2 border-slate-100 ring-4 ring-white/50 animate-slide-up" style={{ maxHeight: '95vh', overflow: 'hidden' }}>
               {isVerifying ? (
                  <div className="p-16 flex-1 flex flex-col items-center justify-center space-y-10 min-h-[400px]">
                     <div className="relative">
                        <div className="absolute inset-0 bg-blue-500 blur-3xl opacity-20 rounded-full animate-pulse"></div>
                        <Loader2 size={120} className="text-blue-600 animate-spin relative z-10" />
                     </div>
                     <h3 className="text-4xl font-black text-slate-900 tracking-tighter italic text-center leading-tight">
                        جاري الأرشفة والتوثيق...<br/><span className="text-blue-600 text-2xl mt-4 inline-block">يرجى الانتظار</span>
                     </h3>
                  </div>
               ) : closingStep === 0 ? (
                 <div className="flex flex-col h-full overflow-hidden">
                     <div className="p-8 sm:p-10 border-b-2 border-slate-50 bg-slate-50/50 shrink-0">
                        <div className="flex justify-between items-center mb-6">
                           <div className="flex items-center gap-5">
                              ...
                              <h3 className="text-3xl font-black tracking-tighter text-slate-900">مراجعة الحالات</h3>
                           </div>
                           <button onClick={() => setIsClosingWizardOpen(false)} className="bg-white p-3 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors border border-slate-200 shadow-sm text-slate-500">
                             <X size={24} />
                           </button>
                        </div>
                        <p className="text-slate-500 font-bold text-lg leading-relaxed">يرجى مراجعة حالات الغياب. يمكنك تحويل حالة الطالب من غائب لمتأخر في حال وصوله للمقعد الآن:</p>
                     </div>
                        ...
                        {myAbsences.length === 0 ? (
                          <div className="py-16 text-center flex flex-col items-center gap-6 h-full justify-center">
                            ...
                            <div className="space-y-2">
                               <h4 className="text-3xl text-slate-800 font-black">قائمة الحضور مكتملة!</h4>
                               <p className="text-slate-500 font-bold text-lg">لا يوجد غياب أو تأخر في هذه اللجنة.</p>
                            </div>
                          </div>
                        ) : (
                          myAbsences.map(a => (
                             <div key={a.id} className="flex flex-col sm:flex-row justify-between items-center p-5 sm:p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all gap-5 group">
                                <div className="flex flex-col flex-1 text-center sm:text-right w-full">
                                  <span className="font-black text-slate-900 text-xl">{a.student_name}</span>
                                  <div className="mt-2">
                                     <span className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest ${a.type === 'ABSENT' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>{a.type === 'ABSENT' ? 'حالة: غائب' : 'حالة: متأخر'}</span>
                                  </div>
                                </div>
                     ...
                     <div className="p-6 sm:p-8 border-t-2 border-slate-50 bg-white shrink-0">
                        <button onClick={() => setClosingStep(1)} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black text-2xl flex items-center justify-center gap-6 shadow-xl hover:bg-slate-800 active:scale-95 transition-all outline-none">
                 ...
                 <div className="flex flex-col h-full overflow-hidden">
                     <div className="p-8 sm:p-10 text-center border-b-2 border-slate-50 shrink-0 relative">
                        <div className="flex items-center justify-between mb-8">
                           <button onClick={() => { setClosingStep(0); setCountError(null); }} className="bg-slate-100 p-4 rounded-full text-slate-500 hover:bg-slate-200 transition-colors outline-none cursor-pointer border border-transparent shadow-sm">
                              <ChevronRight size={24} />
                           </button>
                           <div className="bg-blue-50 border border-blue-100 text-blue-600 px-8 py-2.5 rounded-full text-xl font-black shadow-inner">{myGrades[currentGradeIdx]}</div>
                           <button onClick={() => setIsClosingWizardOpen(false)} className="bg-white p-3 rounded-full hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors shadow-sm border border-slate-200 cursor-pointer">
                              <X size={24} />
                           </button>
                        </div>
                        <h4 className="text-4xl font-black text-slate-900 tracking-tighter mb-4">حصيلة أوراق الإجابة</h4>
                        <p className="text-slate-400 font-bold text-lg uppercase tracking-widest hidden sm:block">قم بعد الأوراق الفعلية داخل مظروف الإجابة</p>
                     </div>
                     
                     <div className="flex-1 overflow-y-auto p-6 sm:p-10 flex flex-col items-center justify-center gap-8 bg-slate-50/20">
                        <div className="text-slate-500 font-bold mb-2 sm:hidden text-center text-sm px-4">قم بعد الأوراق الفعلية وأدخل العدد أدناه:</div>
                        <div className="flex items-center justify-center gap-6 sm:gap-10 w-full max-w-sm mx-auto">
                           <button onClick={() => { setClosingCounts(prev => ({...prev, [myGrades[currentGradeIdx]]: Math.max(0, (prev[myGrades[currentGradeIdx]] || 0) - 1)})); setCountError(null); setIsCountingLocked(false); }} className="w-16 h-16 sm:w-20 sm:h-20 bg-white border-2 border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:text-slate-700 hover:border-slate-300 hover:shadow-md active:scale-90 transition-all shrink-0 cursor-pointer outline-none"><Minus size={36} /></button>
                           ...
                           <button onClick={() => { setClosingCounts(prev => ({...prev, [myGrades[currentGradeIdx]]: (prev[myGrades[currentGradeIdx]] || 0) + 1})); setCountError(null); setIsCountingLocked(false); }} className="w-16 h-16 sm:w-20 sm:h-20 bg-white border-2 border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-300 hover:shadow-md active:scale-90 transition-all shrink-0 cursor-pointer outline-none"><Plus size={36} /></button>
                        </div>
                        ...
                        {countError && (
                           <div className="w-full max-w-sm mx-auto bg-red-50/80 backdrop-blur-sm p-5 rounded-2xl border border-red-200 flex flex-col items-center gap-3 text-red-700 text-center animate-slide-up shadow-sm mt-4">
                              <AlertTriangle className="shrink-0 text-red-500 hidden sm:block" size={28}/>
                     ...
                     <div className="p-6 sm:p-8 border-t-2 border-slate-50 bg-white shrink-0 relative z-10">
                        ...
                        >
                           <span className="mt-1">{currentGradeIdx === myGrades.length - 1 ? 'تأكيد وإرسال للكنترول' : 'التالي'}</span> 
                           {currentGradeIdx === myGrades.length - 1 ? <PackageCheck size={32} /> : <ChevronLeft size={32} />}
                        </button>
                     </div>
                 </div>
               ) : null}
