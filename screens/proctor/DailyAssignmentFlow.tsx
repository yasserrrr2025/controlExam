...
const ProctorDailyAssignmentFlow: React.FC<Props> = ({ 
  ...
  const filteredStudents = useMemo(() => {
    return myStudents.filter(s => s.name.includes(studentSearch) || s.national_id.includes(studentSearch));
  ...
  if (isInitialLoading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 animate-pulse text-slate-400">
        ...
        <p className="font-black text-xl italic text-slate-500">جاري تهيئة مركز العمليات...</p>
      </div>
    );
  }
         ...
         <div className="bg-slate-950 p-10 rounded-[4rem] text-white shadow-2xl relative overflow-hidden border-b-[10px] border-blue-600">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
               <div className="flex items-center gap-8">
                  <div className="w-24 h-24 bg-white rounded-3xl p-1 flex items-center justify-center border-4 border-blue-500/20 shadow-2xl"><img src={APP_CONFIG.LOGO_URL} alt="Logo" className="w-full h-full object-contain" /></div>
                  <div className="text-right">
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto text-right pb-48 px-4 md:px-0">
       <div className="bg-slate-950 text-white h-12 flex items-center overflow-hidden rounded-2xl relative shadow-xl border-b-2 border-blue-600">
          <div className="bg-blue-600 h-full flex items-center px-4 font-black text-[10px] uppercase gap-2 z-10 shrink-0"><Megaphone size={14} className="animate-bounce" /> تعليمات الكنترول</div>
          <div className="flex-1 whitespace-nowrap animate-marquee flex items-center gap-10 font-bold text-sm italic">
             {relevantBroadcasts.length > 0 ? (
                relevantBroadcasts.map((b, i) => (
                  <span key={b.id}>• {b.message} •</span>
                ))
             ) : (
                <>
                  <span>• يرجى التأكد من مطابقة أرقام الجلوس قبل البدء •</span>
                  <span>• يمنع استخدام الهواتف داخل اللجان تماماً •</span>
                  <span>• في حال وجود أي نقص في الأوراق، استخدم بلاغ الطوارئ •</span>
                  <span>• الكنترول جاهز لاستلام المظاريف فور انتهاء الوقت •</span>
                </>
             )}
          </div>
       </div>
       <div className="bg-slate-950 p-8 md:p-10 rounded-[3.5rem] text-white shadow-2xl border-b-[8px] border-blue-600 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/10 blur-3xl"></div>
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
             <div className="flex items-center gap-8">
                <div className="w-24 h-24 bg-blue-600 text-white rounded-[2.5rem] flex flex-col items-center justify-center font-black shadow-2xl relative">
                   <div className="absolute -top-2 -right-2 w-6 h-6 bg-emerald-500 rounded-full border-4 border-slate-950 animate-pulse"></div>
                ...
                <div className="text-right">
                   ...
                   <div className="flex items-center gap-3 mt-2"><span className="bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-lg font-black text-[9px] uppercase flex items-center gap-2"><Activity size={12}/> مباشر الآن</span></div>
                </div>
             </div>
             ...
             <div className="flex gap-4">
                ...
                <div className="bg-white/5 border border-white/10 p-5 rounded-3xl text-center min-w-[100px]"><p className="text-[8px] font-black uppercase text-slate-500 mb-1">الغياب</p><p className="text-2xl font-black text-red-500 tabular-nums">{stats.absent}</p></div>
             </div>
          </div>
       </div>
       <div className="relative group">
          ...
          <input type="text" placeholder="ابحث عن اسم طالب في لجنتك..." className="w-full pr-16 pl-8 py-6 bg-white border-2 border-slate-100 rounded-[2.5rem] font-bold text-xl shadow-xl outline-none focus:border-blue-600 transition-all" value={studentSearch} onChange={e => setStudentSearch(e.target.value)} />
       </div>
            ...
            <div className="space-y-4">
               {myActiveRequests.map(req => (
                 <div key={req.id} className="flex flex-col md:flex-row justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group">
                    <div className="flex items-center gap-4 flex-1"><div className={`w-3 h-3 rounded-full ${req.status === 'PENDING' ? 'bg-amber-500 animate-pulse' : 'bg-blue-500'}`}></div><p className="font-black text-slate-700 text-sm">{req.text}</p></div>
                    <div className="flex items-center gap-4 mt-3 md:mt-0"><span className={`px-4 py-1 rounded-full font-black text-[9px] uppercase tracking-widest ${req.status === 'PENDING' ? 'bg-amber-100 text-amber-600' : 'bg-blue-600 text-white shadow-lg'}`}>{req.status === 'PENDING' ? 'بانتظار المباشرة' : 'المساعد في الطريق إليك'}</span><div className="text-[9px] font-bold text-slate-400 font-mono">{new Date(req.time).toLocaleTimeString('ar-SA', {hour:'2-digit', minute:'2-digit'})}</div></div>
                 </div>
               ))}
            </div>
         </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => setIsReportModalOpen(true)} className="p-8 bg-rose-600 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-xl hover:bg-rose-700 transition-all border-b-[8px] border-rose-800 active:scale-95 group"><Zap size={40} className="group-hover:rotate-12 transition-transform" fill="white" /> بلاغ ميداني عاجل</button>
          <button onClick={() => { setClosingStep(0); setCurrentGradeIdx(0); setIsClosingWizardOpen(true); setCountError(null); setIsCountingLocked(false); }} className="p-8 bg-slate-900 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-xl hover:bg-blue-600 transition-all border-b-[8px] border-slate-950 active:scale-95 group"><PackageCheck size={40} className="group-hover:scale-110 transition-transform" /> إنهاء وإغلاق اللجنة</button>
       </div>
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
         {filteredStudents.map((s: Student) => {
           const status = myAbsences.find(a => a.student_id === s.national_id);
           return (
             <div key={s.id} className={`p-8 rounded-[3.5rem] border-2 transition-all relative overflow-hidden flex flex-col justify-between shadow-2xl min-h-[340px] hover:shadow-blue-100 ${status?.type === 'ABSENT' ? 'bg-red-50/70 border-red-200' : status?.type === 'LATE' ? 'bg-amber-50/70 border-amber-200' : 'bg-white border-slate-50 hover:border-blue-100 group'}`}>
                {status && <div className={`absolute top-0 right-0 w-2 h-full ${status.type === 'ABSENT' ? 'bg-red-500' : 'bg-amber-500'}`}></div>}
                <div className="flex justify-between items-start mb-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner transition-transform ${status?.type === 'ABSENT' ? 'bg-red-600 text-white' : status?.type === 'LATE' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-600 group-hover:text-white'}`}><GraduationCap size={28} /></div>
                  <div className="flex flex-col items-end"><span className={`px-4 py-1.5 rounded-xl font-black text-[10px] shadow-lg uppercase tracking-widest ${status ? (status.type === 'ABSENT' ? 'bg-red-600 text-white' : 'bg-amber-500 text-white') : 'bg-emerald-600 text-white'}`}>{status ? (status.type === 'ABSENT' ? 'غائب' : 'متأخر') : 'حاضر'}</span><span className="text-[9px] font-black text-slate-300 mt-2">رقم الجلوس: {s.seating_number || '---'}</span></div>
                </div>
                <div className="flex-1 text-right space-y-3 px-2"><h4 className="text-2xl font-black text-slate-900 break-words leading-tight group-hover:text-blue-600 transition-colors">{s.name}</h4><div className="flex items-center gap-2"><span className="bg-slate-100 px-3 py-1 rounded-lg text-[9px] font-black text-slate-500">{s.grade} - فصل {s.section}</span></div></div>
                <div className="grid grid-cols-2 gap-3 mt-8">
                   <button onClick={() => toggleStudentStatus(s, 'ABSENT')} className={`py-5 rounded-[1.8rem] font-black text-xs transition-all ${status?.type === 'ABSENT' ? 'bg-red-600 text-white shadow-lg' : 'bg-white/60 text-slate-400 hover:bg-red-50 hover:text-red-600'}`}>رصد غياب</button>
                   <button onClick={() => toggleStudentStatus(s, 'LATE')} className={`py-5 rounded-[1.8rem] font-black text-xs transition-all ${status?.type === 'LATE' ? 'bg-amber-500 text-white shadow-lg' : 'bg-white/60 text-slate-400 hover:bg-red-50 hover:text-red-600'}`}>رصد تأخر</button>
                </div>
             </div>
           );
         })}
          ...
          <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 animate-fade-in no-print">
             ...
             <div className="bg-white w-full max-w-2xl rounded-[4rem] shadow-2xl relative z-10 overflow-hidden border-b-[15px] border-rose-600 animate-slide-up my-auto">
                <div className="bg-rose-600 p-8 text-white flex justify-between items-center relative overflow-hidden">
                   ...
                   <div className="flex items-center gap-4 relative z-10"><div className="bg-white/20 p-3 rounded-2xl"><Zap size={24}/></div><h3 className="text-2xl font-black italic">مركز بلاغات اللجنة {activeCommittee}</h3></div>
                   ...
                   {reportStep === 'CATEGORIES' && (
                     <div className="grid grid-cols-2 gap-4 animate-fade-in">
                        {[
                          ...
                          <button key={cat.id} onClick={() => { setSelectedCategory(cat.id); if (cat.id === 'HEALTH') setSelectedCategory('HEALTH'); else setReportStep(cat.step as any); }} className="p-8 bg-white border-2 border-slate-100 rounded-[2.5rem] flex flex-col items-center gap-4 hover:border-rose-500 hover:shadow-xl transition-all group active:scale-95"><div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform ${cat.color}`}><cat.icon size={32}/></div><span className="font-black text-slate-800 text-sm">{cat.label}</span></button>
                        ))}
                        {selectedCategory === 'HEALTH' && <button onClick={handleUrgentReport} className="col-span-2 py-6 bg-red-600 text-white rounded-[2rem] font-black text-xl shadow-xl mt-4 animate-pulse">تأكيد إرسال البلاغ الصحي العاجل</button>}
                     </div>
                   )}
                   {reportStep === 'SELECT_STUDENTS' && (
                     <div className="space-y-6 animate-fade-in"><div className="flex items-center gap-3 mb-4"><Users className="text-blue-600" /><h4 className="text-xl font-black text-slate-800">اختر الطلاب المحتاجين للورقة:</h4></div><div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">{myStudents.map(s => (<button key={s.id} onClick={() => setSelectedStudentIds(prev => prev.includes(s.national_id) ? prev.filter(id => id !== s.national_id) : [...prev, s.national_id])} className={`p-4 rounded-2xl flex items-center justify-between border-2 transition-all ${selectedStudentIds.includes(s.national_id) ? 'bg-blue-50 border-blue-600' : 'bg-slate-50 border-slate-100'}`}><span className="font-bold text-slate-800">{s.name}</span>{selectedStudentIds.includes(s.national_id) ? <CheckCircle2 className="text-blue-600"/> : <div className="w-6 h-6 rounded-full border-2 border-slate-200"></div>}</button>))}</div><div className="flex gap-3"><button onClick={handleUrgentReport} disabled={selectedStudentIds.length === 0} className="flex-1 py-6 bg-rose-600 text-white rounded-[2rem] font-black text-xl shadow-xl disabled:opacity-50">إرسال الطلب ({selectedStudentIds.length})</button><button onClick={() => setReportStep('CATEGORIES')} className="px-8 bg-slate-100 text-slate-600 rounded-[2rem] font-black">رجوع</button></div></div>
                   )}
                   {reportStep === 'SELECT_TEACHER' && (
                     <div className="space-y-6 animate-fade-in"><div className="flex items-center gap-3 mb-4"><UserCog className="text-purple-600" /><h4 className="text-xl font-black text-slate-800">اختر المعلم المراد استدعاؤه:</h4></div><div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">{users.filter(u => u.id !== user.id).map(u => (<button key={u.id} onClick={() => setOtherText(u.full_name)} className={`p-4 rounded-2xl flex items-center justify-between border-2 transition-all ${otherText === u.full_name ? 'bg-purple-50 border-purple-600' : 'bg-slate-50 border-slate-100'}`}><div className="text-right"><p className="font-bold text-slate-800">{u.full_name}</p><p className="text-[9px] font-black text-slate-400 uppercase">{ROLES_ARABIC[u.role]}</p></div>{otherText === u.full_name ? <CheckCircle2 className="text-purple-600"/> : <div className="w-6 h-6 rounded-full border-2 border-slate-200"></div>}</button>))}</div><div className="flex gap-3"><button onClick={handleUrgentReport} disabled={!otherText} className="flex-1 py-6 bg-rose-600 text-white rounded-[2rem] font-black text-xl shadow-xl disabled:opacity-50">إرسال الاستدعاء</button><button onClick={() => setReportStep('CATEGORIES')} className="px-8 bg-slate-100 text-slate-600 rounded-[2rem] font-black">رجوع</button></div></div>
                   )}
                   {reportStep === 'INPUT_QUANTITY' && (
                     <div className="space-y-8 animate-fade-in text-center"><h4 className="text-xl font-black text-slate-800">حدد الكمية المطلوبة بدقة:</h4><div className="flex items-center justify-center gap-10"><button onClick={() => setQuantity(prev => Math.max(1, prev - 1))} className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 active:scale-90 transition-transform"><Minus size={32}/></button><span className="text-7xl font-black text-slate-900 tabular-nums">{quantity}</span><button onClick={() => setQuantity(prev => prev + 1)} className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 active:scale-90 transition-transform"><Plus size={32}/></button></div><div className="flex gap-3 mt-8"><button onClick={handleUrgentReport} className="flex-1 py-6 bg-rose-600 text-white rounded-[2rem] font-black text-xl shadow-xl">تأكيد الطلب</button><button onClick={() => setReportStep('CATEGORIES')} className="px-8 bg-slate-100 text-slate-600 rounded-[2rem] font-black">رجوع</button></div></div>
                   )}
                   {reportStep === 'OTHER' && (
                     <div className="space-y-6 animate-fade-in"><h4 className="text-xl font-black text-slate-800">اكتب تفاصيل البلاغ للإدارة:</h4><textarea value={otherText} onChange={e => setOtherText(e.target.value)} className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold text-lg h-32 outline-none focus:border-rose-600" placeholder="اكتب هنا بوضوح..."></textarea><div className="flex gap-3"><button onClick={handleUrgentReport} disabled={!otherText.trim()} className="flex-1 py-6 bg-rose-600 text-white rounded-[2rem] font-black text-xl shadow-xl disabled:opacity-50">إرسال البلاغ</button><button onClick={() => setReportStep('CATEGORIES')} className="px-8 bg-slate-100 text-slate-600 rounded-[2rem] font-black">رجوع</button></div></div>
                   )}
                </div>
             </div>
          </div>
