  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto text-right pb-48 px-4 md:px-0">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
             ...
             <div className="flex-1 max-w-md mx-6 hidden lg:block">
                <div className="flex justify-between mb-2 px-2">
                   <span className="text-[9px] font-black text-slate-500 uppercase">مؤشر رصد الحضور المباشر</span>
                   <span className="text-[9px] font-black text-blue-400">{stats.progress}%</span>
                </div>
                <div className="h-2.5 bg-white/5 rounded-full overflow-hidden p-0.5 border border-white/10">
                   <div className="h-full bg-blue-600 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(37,99,235,0.5)]" style={{ width: `${stats.progress}%` }}></div>
                </div>
             </div>

             <div className="flex gap-4">
                <div className="bg-white/5 border border-white/10 p-5 rounded-3xl text-center min-w-[100px]">
                   <p className="text-[8px] font-black uppercase text-slate-500 mb-1">طلاب اللجنة</p>
       ...
       <div className="relative group">
          ...
          <input 
            type="text" 
            placeholder="ابحث عن اسم طالب في لجنتك..." 
            className="w-full pr-16 pl-8 py-6 bg-white border-2 border-slate-100 rounded-[2.5rem] font-bold text-xl shadow-xl outline-none focus:border-blue-600 transition-all"
            value={studentSearch}
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => setIsReportModalOpen(true)} className="p-8 bg-rose-600 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-xl hover:bg-rose-700 transition-all border-b-[8px] border-rose-800 active:scale-95 group">
             <Zap size={40} className="group-hover:rotate-12 transition-transform" fill="white" /> بلاغ ميداني عاجل
          </button>
          <button onClick={() => { setClosingStep(0); setCurrentGradeIdx(0); setIsClosingWizardOpen(true); setCountError(null); setIsCountingLocked(false); }} className="p-8 bg-slate-900 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-xl hover:bg-blue-600 transition-all border-b-[8px] border-slate-950 active:scale-95 group">
             <PackageCheck size={40} className="group-hover:scale-110 transition-transform" /> إنهاء وإغلاق اللجنة
          </button>
       </div>
         ...
         {filteredStudents.map((s: Student) => {
           ...
           return (
             <div key={s.id} className={`p-8 rounded-[3.5rem] border-2 transition-all relative overflow-hidden flex flex-col justify-between shadow-2xl min-h-[340px] hover:shadow-blue-100 ${status?.type === 'ABSENT' ? 'bg-red-50/70 border-red-200' : status?.type === 'LATE' ? 'bg-amber-50/70 border-amber-200' : 'bg-white border-slate-50 hover:border-blue-100 group'}`}>
                {status && (
                  <div className={`absolute top-0 right-0 w-2 h-full ${status.type === 'ABSENT' ? 'bg-red-500' : 'bg-amber-500'}`}></div>
                )}
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-[9px] font-black text-slate-300 mt-2">رقم الجلوس: {s.seating_number || '---'}</span>
                  </div>
                </div>
                <div className="flex-1 text-right space-y-3 px-2">
                   <h4 className="text-2xl font-black text-slate-900 break-words leading-tight group-hover:text-blue-600 transition-colors">{s.name}</h4>
                ...
                <div className="grid grid-cols-2 gap-3 mt-8">
                   ...
                   <button onClick={() => toggleStudentStatus(s, 'LATE')} className={`py-5 rounded-[1.8rem] font-black text-xs transition-all ${status?.type === 'LATE' ? 'bg-amber-500 text-white shadow-lg' : 'bg-white/60 text-slate-400 hover:bg-amber-50 hover:text-amber-600'}`}>رصد تأخر</button>
                </div>
             </div>
           );
         })}
         {filteredStudents.length === 0 && (
           <div className="col-span-full py-20 text-center text-slate-300 italic flex flex-col items-center gap-4">
              <UserSearch size={64} className="opacity-20" />
              <p className="text-xl font-black">لا يوجد طلاب يطابقون بحثك</p>
           </div>
         )}
       </div>
                   ...
                   {reportStep === 'CATEGORIES' && (
                     <div className="grid grid-cols-2 gap-4 animate-fade-in">
                        {[
                          ...
                          }} className="p-8 bg-white border-2 border-slate-100 rounded-[2.5rem] flex flex-col items-center gap-4 hover:border-rose-500 hover:shadow-xl transition-all group active:scale-95">
